...
#define array_reserve(self, new_capacity)                                      \
	((self)->contents =                                                        \
		 _array__reserve((void *)(self)->contents, &(self)->capacity,          \
...
#define array_push(self, element)                                              \
	do {                                                                       \
		(self)->contents =                                                     \
			_array__grow((void *)(self)->contents, (self)->size,               \
						 &(self)->capacity, 1, array_elem_size(self));         \
...
#define array_grow_by(self, count)                                             \
	do {                                                                       \
		if ((count) == 0)                                                      \
			break;                                                             \
		(self)->contents =                                                     \
			_array__grow((self)->contents, (self)->size, &(self)->capacity,    \
...
// aliasing violations that arises when the _entire_ `Array` struct is passed as
// `Array(void)*`.
//
// The `Array` type itself was not altered as a solution in order to avoid
// breakage with existing consumers (in particular, parsers with external
// scanners).
...
static inline void *_array__assign(void *self_contents, uint32_t *self_size,
								   ...
								   const void *other_contents,
								   uint32_t other_size, size_t element_size)
{
	void *new_contents =
		_array__reserve(self_contents, self_capacity, element_size, other_size);
...
static inline struct Swap _array__swap(void *self_contents, uint32_t *self_size,
									   uint32_t *self_capacity,
									   void *other_contents,
									   uint32_t *other_size,
...
static inline void *_array__grow(void *contents, uint32_t size,
								   ...
								   uint32_t new_count, const void *elements)
{
	...
	void *new_contents =
		_array__reserve(self_contents, capacity, element_size, new_size);
